...
import { BenchmarkResult, BenchmarkResults } from './benchmark_result';
import path from 'node:path';
import extractCatch2Result from './extract_catch2';
...
export async function extractResult(config: Config): Promise<Benchmark> {
    ...
    switch (tool) {
        ...
        case 'googlecpp':
            ...
            break;
        case 'catch2':
            benches = extractCatch2Result(output, path.parse(config.outputFilePath).ext);
            break;
        case 'julia':
